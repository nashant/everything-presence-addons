...
| D007 | M003 | process | Branch strategy for M003 | Sub-branches of `feat/multi-device-rooms` per task: `feat/multi-device-rooms/M003/S01/T01` etc. | Keeps work isolated per task, allows incremental review. `feat/multi-device-rooms` is the integration branch (branched from `feat/room-first-ux`). | No |
