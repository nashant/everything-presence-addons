...
| K008 | M004/S02 | backend | HA MQTT discovery derives entity_id from `object_id` field, NOT `unique_id`. If your object_id is `zone_0_occupancy`, the entity_id becomes `binary_sensor.zone_0_occupancy` — not `binary_sensor.ep_room_bedroom_01_zone_0_occupancy`. The unique_id is only for internal registry deduplication. Any code looking up entities after MQTT discovery must use object_id-derived IDs. |
| K009 | M004/S02 | backend | sanitizeForMqtt returns `'unnamed'` for empty/all-special-char inputs rather than throwing. Discovery payload builder uses pure functions — no side effects, fully unit-testable. Topic namespace is `ep_room/` (distinct from `ep_mock/` for mock devices). |
| K010 | M004/S02 | backend | removeRoomDevice publishes entity config empties BEFORE availability removal. This ordering gives HA a clean teardown — entities disappear before the device availability goes offline. |
| K011 | M004/S03 | testing | Vitest 3.x changed `vi.fn<>` generic syntax. Use function-signature generics: `vi.fn<(arg: Type) => Return>()` instead of the v2 positional `vi.fn<[ArgTypes], Return>()`. The old syntax produces TS2558 "Expected 0-1 type arguments, but got 2". |
| K012 | M004/S03 | backend | `npx tsc --noEmit` must run from `backend/` or `frontend/` subdirectory (where tsconfig.json lives), NOT from the monorepo root `everything-presence-mmwave-configurator/`. Running from root prints help text and exits 1 with no useful error. |
