...
function createLiveRouter(readTransport, writeClient, profileLoader) {
    ...
    router.get('/:deviceId/state', async (req, res) => {
        try {
            ...
            const profile = profileId ? profileLoader.getProfileById(profileId) : null;
            if (!profile) {
            ...
            const normalizeState = (state) => (typeof state === 'string' ? state.toLowerCase() : null);
            ...
            const markAvailability = (key, state) => {
                if (!state)
                    return;
            ...
            const getEntityState = async (mappingKey, template) => {
                ...
                if (!entityId) {
                    entityId = entityResolver_1.EntityResolver.resolve(entityMappings, deviceName, mappingKey, template);
                }
                if (!entityId)
                    return null;
                try {
            ...
            if (capabilities?.tracking) {
                ...
                const getTargetState = async (targetNum, property) => {
                    let entityId = null;
                    ...
                    if (hasDeviceMapping) {
                        ...
                        if (targetSet && targetSet[property]) {
                            entityId = targetSet[property];
                        }
                    }
                    // Fallback to legacy resolution
                    if (!entityId) {
                        entityId = entityResolver_1.EntityResolver.resolveTargetEntity(entityMappings, deviceName, targetNum, property);
                    }
                    if (!entityId)
                        return null;
                    try {
                ...
                for (let i = 1; i <= 3; i++) {
                    const target = { id: i, x: null, y: null };
                    // Fetch active status
                    ...
                    if (xState && !isUnavailableState(xState.state)) {
                        ...
                        if (!isNaN(x)) {
                            ...
                            x = convertToMm(x, xState.attributes?.unit_of_measurement);
                            target.x = x;
                        }
                    }
                    ...
                    if (yState && !isUnavailableState(yState.state)) {
                        ...
                        if (!isNaN(y)) {
                            // Convert to mm if unit is imperial
                            y = convertToMm(y, yState.attributes?.unit_of_measurement);
                            target.y = y;
                        }
                    }
                    ...
                    if (distanceState && !isUnavailableState(distanceState.state)) {
                        ...
                        if (!isNaN(distance)) {
                            distance = convertToMm(distance, distanceState.attributes?.unit_of_measurement);
            ...
            logger_1.logger.error({ err }, 'Failed to get live state');
    ...
    router.post('/:deviceId/entity', async (req, res) => {
        try {
            ...
            logger_1.logger.info({ deviceId, entityId, value }, 'Updating device entity');
            // Determine the service to call based on entity type
            const [domain] = entityId.split('.');
            // Use write client convenience methods based on domain
            switch (domain) {
                case 'select':
                    await writeClient.setSelectEntity(entityId, value);
                    break;
                case 'number':
                    await writeClient.setNumberEntity(entityId, value);
                    break;
                case 'switch':
                    await writeClient.setSwitchEntity(entityId, value);
                    break;
                case 'input_boolean':
                    await writeClient.setInputBooleanEntity(entityId, value);
                    break;
                default:
            ...
            logger_1.logger.error({ err }, 'Failed to update entity');
            res.status(500).json({ error: err instanceof Error ? err.message : 'Unknown error' });
        }
    });
    /**
    ...
    router.get('/ha/states/:entityId(*)', async (req, res) => {
        try {
            ...
            logger_1.logger.error({ err }, 'Failed to get entity state');
            res.status(500).json({ error: err instanceof Error ? err.message : 'Unknown error' });
        }
    });
    return router;
