...
exports.createFirmwareRouter = void 0;
const express_1 = require("express");
const firmwareService_1 = require("../domain/firmwareService");
...
const firmwareMigrationStorage_1 = require("../config/firmwareMigrationStorage");
const createFirmwareRouter = (deps) => {
    const router = (0, express_1.Router)();
    const firmwareService = new firmwareService_1.FirmwareService({
        config: deps.config,
    ...
    router.get('/migration/active', (_req, res) => {
        const state = firmwareMigrationStorage_1.firmwareMigrationStorage.getActive();
        res.json({ state });
    });
    /**
    ...
    router.get('/migration/:deviceId', (req, res) => {
        ...
        const state = firmwareMigrationStorage_1.firmwareMigrationStorage.get(deviceId);
        res.json({ state });
    });
    /**
    ...
    router.put('/migration/:deviceId', (req, res) => {
        ...
        const backupId = req.body?.backupId ?? undefined;
        const preparedVersion = req.body?.preparedVersion ?? undefined;
        const lastError = req.body?.lastError ?? undefined;
        const state = firmwareMigrationStorage_1.firmwareMigrationStorage.upsert(deviceId, {
            phase,
            backupId,
            preparedVersion,
            lastError,
    ...
    router.delete('/migration/:deviceId', (req, res) => {
        const { deviceId } = req.params;
        firmwareMigrationStorage_1.firmwareMigrationStorage.clear(deviceId);
        res.json({ ok: true });
    });
    /**
    ...
    router.get('/settings', (_req, res) => {
        try {
            ...
            const effectiveCacheKeepCount = typeof settings.cacheKeepCount === 'number' && settings.cacheKeepCount > 0
            ...
            res.json({
                ...
                defaultIndexUrls: firmware_1.DEFAULT_FIRMWARE_INDEX_URLS,
            });
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Failed to get firmware settings');
    ...
    router.put('/settings', (req, res) => {
        try {
            ...
            if (Array.isArray(req.body?.firmwareIndexUrls)) {
                const urls = req.body.firmwareIndexUrls.filter((url) => typeof url === 'string' && url.trim());
            ...
            const settings = firmwareStorage_1.firmwareStorage.saveSettings(updates);
            const autoDetectedIp = firmwareService.getLanIp();
            const effectiveCacheKeepCount = typeof settings.cacheKeepCount === 'number' && settings.cacheKeepCount > 0
                ? Math.floor(settings.cacheKeepCount)
                : deps.config.maxVersionsPerDevice;
            ...
            logger_1.logger.error({ error }, 'Failed to update firmware settings');
    ...
    router.post('/prepare', async (req, res) => {
        try {
            ...
            const deviceConfig = await firmwareService.getDeviceConfig(deviceModel, firmwareVersion, deviceId);
            ...
            logger_1.logger.info({ deviceId, manifestUrl }, 'Preparing firmware');
            ...
            logger_1.logger.error({ error }, 'Failed to prepare firmware');
            res.status(500).json({
    ...
    router.post('/update/:deviceId', async (req, res) => {
        try {
            ...
            logger_1.logger.info({ deviceId, token, localManifestUrl }, 'Triggering device update');
            ...
            logger_1.logger.error({ error }, 'Failed to trigger device update');
            res.status(500).json({
    ...
    router.get('/update-status/:deviceId', async (req, res) => {
        try {
            ...
            const updateEntityId = deviceEntityService_1.deviceEntityService.getEntityId(deviceId, 'firmwareUpdateEntity');
            if (!updateEntityId) {
                return res.status(404).json({
            ...
            logger_1.logger.error({ error }, 'Failed to fetch update entity status');
    ...
    router.get('/cache', (_req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to list firmware cache');
    ...
    router.get('/cache/:deviceId', (req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to list device firmware cache');
    ...
    router.delete('/cache/:deviceId/:token', (req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to delete firmware cache entry');
    ...
    router.get('/index', async (_req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to fetch firmware indexes');
    ...
    router.post('/index/refresh', async (_req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to refresh firmware indexes');
    ...
    router.post('/device-config', async (req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to get device config');
            res.status(500).json({
    ...
    router.post('/available', async (req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to get available updates');
            res.status(500).json({
    ...
    router.post('/validate', async (req, res) => {
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to validate firmware');
            res.status(500).json({
    ...
    router.post('/auto-prepare', async (req, res) => {
        try {
            ...
            const baseUrl = settings.firmwareIndexUrls?.[0]?.replace('/firmware-index.json', '') ||
                firmware_1.DEFAULT_FIRMWARE_INDEX_URLS[deviceConfig.model]?.replace('/firmware-index.json', '');
            if (!baseUrl) {
            ...
            logger_1.logger.error({ error }, 'Failed to auto-prepare firmware');
            res.status(500).json({
...
exports.createFirmwareRouter = createFirmwareRouter;
