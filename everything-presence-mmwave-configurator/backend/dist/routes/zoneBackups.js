...
exports.createZoneBackupsRouter = void 0;
const express_1 = require("express");
const crypto_1 = __importDefault(require("crypto"));
const zoneReader_1 = require("../ha/zoneReader");
const zoneWriter_1 = require("../ha/zoneWriter");
const zoneBackupStorage_1 = require("../config/zoneBackupStorage");
...
const parseEntityMappings = (value) => {
    if (!value)
        return undefined;
...
const parseZoneIndex = (id) => {
    const match = id.match(/(\d+)/);
    return match ? parseInt(match[1], 10) : 0;
};
const sortZonesByIndex = (zones) => [...zones].sort((a, b) => parseZoneIndex(a.id) - parseZoneIndex(b.id));
const isValidRect = (zone) => Number.isFinite(zone.width) && Number.isFinite(zone.height) && zone.width > 0 && zone.height > 0;
const createZoneBackupsRouter = (deps) => {
    const router = (0, express_1.Router)();
    const zoneReader = new zoneReader_1.ZoneReader(deps.readTransport);
    const zoneWriter = new zoneWriter_1.ZoneWriter(deps.writeClient);
    /**
    ...
    router.get('/', (req, res) => {
        ...
        const backups = zoneBackupStorage_1.zoneBackupStorage.listBackups();
    ...
    router.get('/:backupId', (req, res) => {
        ...
        const backup = zoneBackupStorage_1.zoneBackupStorage.getBackup(backupId);
        if (!backup) {
    ...
    router.delete('/:backupId', (req, res) => {
        const { backupId } = req.params;
        const deleted = zoneBackupStorage_1.zoneBackupStorage.deleteBackup(backupId);
        if (!deleted) {
            return res.status(404).json({ message: 'Backup not found' });
        }
    ...
    router.post('/', async (req, res) => {
        ...
        const prefix = entityNamePrefix ||
            deviceEntityService_1.deviceEntityService.getDeviceNamePrefix(deviceId) ||
            undefined;
        if (!prefix) {
            return res.status(400).json({ message: 'entityNamePrefix is required when device mapping is missing' });
        }
        try {
            const zones = await zoneReader.readZones(profile.entityMap, prefix, entityMappings, deviceId);
            ...
            zoneBackupStorage_1.zoneBackupStorage.saveBackup(backup);
            return res.json({ backup });
        }
        catch (error) {
            logger_1.logger.error({ error, deviceId }, 'Failed to create zone backup');
    ...
    router.post('/import', (req, res) => {
        ...
        for (const entry of candidates) {
            ...
            const zones = Array.isArray(entry.zones) ? entry.zones : undefined;
        ...
        zoneBackupStorage_1.zoneBackupStorage.saveBackups(valid);
    ...
    router.post('/:backupId/restore', async (req, res) => {
        ...
        const deviceId = req.body?.deviceId ?? backup.deviceId;
        const profileId = req.body?.profileId ?? backup.profileId;
        const entityNamePrefix = req.body?.entityNamePrefix;
        const entityMappings = parseEntityMappings(req.body?.entityMappings);
        if (!deviceId) {
            return res.status(400).json({ message: 'deviceId is required' });
        ...
        const regularZones = sortZonesByIndex(backup.zones.filter((zone) => zone.type === 'regular' && isValidRect(zone))).slice(0, maxZones);
        const exclusionZones = sortZonesByIndex(backup.zones.filter((zone) => zone.type === 'exclusion' && isValidRect(zone))).slice(0, maxExclusion);
        const entryZones = sortZonesByIndex(backup.zones.filter((zone) => zone.type === 'entry' && isValidRect(zone))).slice(0, maxEntry);
        ...
        const regularIdMap = new Map();
        const exclusionIdMap = new Map();
        const entryIdMap = new Map();
        const polygons = [
            ...regularZones.map((zone, idx) => {
                const newId = `Zone ${idx + 1}`;
                regularIdMap.set(zone.id, newId);
                return { ...(0, polygonUtils_1.rectToPolygon)(zone), id: newId };
            }),
        ...
        ].filter((zone) => (0, polygonUtils_1.isValidPolygon)(zone.vertices));
        try {
            await zoneWriter.setPolygonMode(profile.entityMap, prefix, true, entityMappings, deviceId);
            const result = await zoneWriter.applyPolygonZones(profile.entityMap, polygons, prefix, entityMappings, deviceId);
            ...
            if (backup.zoneLabels) {
                ...
                if (!mapping) {
                    ...
                    const remappedLabels = {};
                    for (const [oldId, label] of Object.entries(backup.zoneLabels)) {
                        const remapped = regularIdMap.get(oldId) ?? exclusionIdMap.get(oldId) ?? entryIdMap.get(oldId) ?? null;
                        if (remapped) {
                            remappedLabels[remapped] = label;
                        }
                    }
                    const merged = { ...(mapping.zoneLabels ?? {}), ...remappedLabels };
                    await deviceMappingStorage_1.deviceMappingStorage.saveMapping({
                        ...mapping,
                        zoneLabels: merged,
            ...
            logger_1.logger.error({ error, backupId, deviceId }, 'Failed to restore zone backup');
            return res.status(500).json({ message: 'Failed to restore zone backup' });
        }
    });
    return router;
};
exports.createZoneBackupsRouter = createZoneBackupsRouter;
