...
exports.createCustomAssetsRouter = void 0;
const express_1 = require("express");
const uuid_1 = require("uuid");
const storage_1 = require("../config/storage");
const createCustomAssetsRouter = () => {
    const router = (0, express_1.Router)();
    // ==================== CUSTOM FLOOR MATERIALS ====================
    // List all custom floor materials
    router.get('/floors', (_req, res) => {
        const floors = storage_1.storage.listCustomFloors();
        res.json({ floors });
    });
    ...
    router.post('/floors', (req, res) => {
        ...
        const floor = {
            id: `custom-${(0, uuid_1.v4)().slice(0, 8)}`,
        ...
        storage_1.storage.saveCustomFloor(floor);
        res.status(201).json({ floor });
    });
    // Update a custom floor material
    router.put('/floors/:id', (req, res) => {
        const { id } = req.params;
        const existing = storage_1.storage.getCustomFloor(id);
        if (!existing) {
        ...
        storage_1.storage.saveCustomFloor(updated);
        res.json({ floor: updated });
    });
    // Delete a custom floor material
    router.delete('/floors/:id', (req, res) => {
        const { id } = req.params;
        const deleted = storage_1.storage.deleteCustomFloor(id);
        if (!deleted) {
            return res.status(404).json({ error: 'Custom floor not found' });
        }
    ...
    router.get('/furniture', (_req, res) => {
        const furniture = storage_1.storage.listCustomFurniture();
        res.json({ furniture });
    });
    ...
    router.post('/furniture', (req, res) => {
        ...
        storage_1.storage.saveCustomFurniture(furniture);
        res.status(201).json({ furniture });
    });
    // Update a custom furniture type
    router.put('/furniture/:id', (req, res) => {
        const { id } = req.params;
        const existing = storage_1.storage.getCustomFurniture(id);
        if (!existing) {
        ...
        storage_1.storage.saveCustomFurniture(updated);
        res.json({ furniture: updated });
    });
    // Delete a custom furniture type
    router.delete('/furniture/:id', (req, res) => {
        const { id } = req.params;
        const deleted = storage_1.storage.deleteCustomFurniture(id);
        if (!deleted) {
            return res.status(404).json({ error: 'Custom furniture not found' });
        }
        res.json({ success: true });
...
exports.createCustomAssetsRouter = createCustomAssetsRouter;
