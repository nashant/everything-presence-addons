...
exports.createDevicesRouter = void 0;
...
const createDevicesRouter = (deps) => {
    ...
    const readinessLogThrottle = new Map();
    ...
    const isReadinessUnavailableState = (state) => {
        const normalized = normalizeStateValue(state);
    ...
    router.get('/:deviceId/readiness', async (req, res) => {
        ...
        const entityNamePrefix = typeof req.query?.entityNamePrefix === 'string' ? req.query.entityNamePrefix : null;
        ...
        try {
            ...
            const registryByObjectId = new Map();
            ...
            if (requireMode === 'polygon') {
                ...
                if (profileId && profileId !== mapping.profileId) {
                    logger_1.logger.warn({ deviceId, requested: profileId, mapped: mapping.profileId }, '[EP migration] readiness: profileId mismatch; using mapped profileId');
                }
                ...
                const mappedPolygonIds = [];
                const missingMappedKeys = [];
                for (let i = 1; i <= requiredZones; i++) {
                    const entityId = deviceEntityService_1.deviceEntityService.getPolygonZoneEntity(deviceId, 'polygon', i);
                    if (entityId)
                        mappedPolygonIds.push(entityId);
                    else
                        missingMappedKeys.push(`polygonZone${i}`);
                }
                for (let i = 1; i <= requiredExclusions; i++) {
                    const entityId = deviceEntityService_1.deviceEntityService.getPolygonZoneEntity(deviceId, 'polygonExclusion', i);
                    if (entityId)
                        mappedPolygonIds.push(entityId);
                    else
                        missingMappedKeys.push(`polygonExclusion${i}`);
                }
                for (let i = 1; i <= requiredEntries; i++) {
                    const entityId = deviceEntityService_1.deviceEntityService.getPolygonZoneEntity(deviceId, 'polygonEntry', i);
                    if (entityId)
                        mappedPolygonIds.push(entityId);
                    else
                        missingMappedKeys.push(`polygonEntry${i}`);
                }
            ...
            const checkedEntityIds = requiredEntityIds.length > 0
                ? requiredEntityIds
                : enabledDeviceEntries.slice(0, 12).map((entry) => entry.entity_id);
            const activeTransport = readTransport.activeTransport ?? null;
            ...
            const missingRegistryEntityIds = requireMode === 'polygon'
            ...
            if (shouldFetchStates) {
                try {
                    states = (await readTransport.getStates(checkedEntityIds));
                }
                catch (error) {
                    logger_1.logger.warn({ error, deviceId }, 'Failed to fetch states for readiness probe');
                }
            }
            // In polygon mode:
            ...
            const missingStateEntityIds = requireMode === 'polygon'
                ? (shouldFetchStates ? checkedEntityIds.filter((id) => Boolean(registryById.get(id)) && !states.get(id)) : [])
                : checkedEntityIds.filter((id) => Boolean(deviceEntriesById.get(id)) && !states.get(id));
            const missingEntityIds = requireMode === 'polygon'
                ? requiredEntityIds.filter((id) => !registryById.get(id) && !states.get(id))
                : checkedEntityIds.filter((id) => !states.get(id));
            const unavailableEntityIds = requireMode === 'polygon'
            ...
            const disabledEntityIds = requireMode === 'polygon'
            ...
            const availableEntityCount = requireMode === 'polygon'
            ...
            const ready = requireMode === 'polygon'
            ...
            const registryPrefixPolygonCandidates = requireMode === 'polygon' && entityNamePrefix
                ? entityRegistry
                    .map((entry) => entry.entity_id)
                    .filter((id) => id.includes(entityNamePrefix) && id.includes('polygon'))
                    .slice(0, 30)
                : [];
            const devicePolygonCandidates = requireMode === 'polygon'
                ? deviceEntries
                    .map((entry) => entry.entity_id)
                    .filter((id) => id.includes('polygon'))
                    .slice(0, 30)
                : [];
            const stuckPolygonReadiness = requireMode === 'polygon' &&
            ...
            if (stuckPolygonReadiness) {
                ...
                if (Date.now() - last > 10_000) {
                    ...
                    logger_1.logger.info({
                        ...
                        note: 'Polygon readiness is failing because HA read transport cannot see expected entities in registry or states. Candidates/matches may reveal ID/domain mismatch.',
            ...
            const debugInfo = {
                ...
                perEntity: checkedEntityIds.map((id) => {
                    ...
                    const stateStatus = stateValue == null ? 'missing' : normalized === 'unavailable' ? 'unavailable' : normalized === 'unknown' ? 'unknown' : 'available';
                    return {
                        entityId: id,
                        ...
                        stateStatus,
                        statePreview: debug && typeof stateValue === 'string' ? (stateValue.length > 80 ? `${stateValue.slice(0, 80)}…` : stateValue) : null,
                    };
                }),
            };
            return res.json({
            ...
            logger_1.logger.error({ error, deviceId }, 'Readiness probe failed');
    ...
    router.get('/:deviceId/zone-availability', async (req, res) => {
        ...
        if (entityMappingsJson && typeof entityMappingsJson === 'string') {
            try {
                ...
                logger_1.logger.warn('Invalid entityMappings JSON in zone-availability query');
            }
        }
        ...
        try {
            const entityRegistry = await readTransport.listEntityRegistry();
            const prefix = entityNamePrefix;
            const availability = {};
            const polygonAvailability = {};
            const registryById = new Map(entityRegistry.map((entry) => [entry.entity_id, entry]));
            const zoneEntitiesByKey = new Map();
            const polygonEntitiesByKey = new Map();
            // Helper to resolve zone entity set - tries device mapping first, then legacy
            const resolveZoneSet = (type, index, groupKey, key, mapping) => {
                if (hasDeviceMapping) {
                    const zoneSet = deviceEntityService_1.deviceEntityService.getZoneEntitySet(deviceId, type, index);
                    if (zoneSet)
                        return zoneSet;
                }
                return entityResolver_1.EntityResolver.resolveZoneEntitySet(entityMappings, prefix, groupKey, key, mapping);
            };
            // Helper to extract zone index from key like "zone1" or "entry2"
            const getZoneIndex = (key) => {
            ...
            if (zoneMap.zoneConfigEntities || hasDeviceMapping) {
                ...
                for (const zoneKey of zoneKeys) {
                    const zoneEntities = (zoneMap.zoneConfigEntities?.[zoneKey] || {});
            ...
            if (zoneMap.exclusionZoneConfigEntities || hasDeviceMapping) {
                ...
                for (const zoneKey of zoneKeys) {
                    const zoneEntities = (zoneMap.exclusionZoneConfigEntities?.[zoneKey] || {});
            ...
            if (zoneMap.entryZoneConfigEntities || hasDeviceMapping) {
                ...
                for (const zoneKey of zoneKeys) {
                    const zoneEntities = (zoneMap.entryZoneConfigEntities?.[zoneKey] || {});
            ...
            const resolvePolygonEntity = (type, groupKey, key, template) => {
                ...
                return entityResolver_1.EntityResolver.resolvePolygonZoneEntity(entityMappings, prefix, groupKey, key, template);
            };
            const addPolygonAvailability = (type, groupKey, labelPrefix, maxCount) => {
                const map = zoneMap[groupKey];
            ...
            const zoneEntityIds = Array.from(new Set([
            ...
            if (zoneEntityIds.length > 0) {
                try {
                    ...
                    logger_1.logger.warn({ error }, 'Failed to fetch zone entity states for availability checks');
                }
            }
            const buildAvailability = (entityIds) => {
            ...
            let polygonZone1Entity = null;
            if (hasDeviceMapping) {
                polygonZone1Entity = deviceEntityService_1.deviceEntityService.getPolygonZoneEntity(deviceId, 'polygon', 1);
            }
            if (!polygonZone1Entity && zoneMap.polygonZoneEntities?.zone1) {
                polygonZone1Entity = entityResolver_1.EntityResolver.resolvePolygonZoneEntity(entityMappings, prefix, 'polygonZoneEntities', 'zone1', zoneMap.polygonZoneEntities.zone1);
            }
            ...
            let entryZone1BeginX = null;
            if (hasDeviceMapping) {
                const entryZoneSet = deviceEntityService_1.deviceEntityService.getZoneEntitySet(deviceId, 'entry', 1);
                entryZone1BeginX = entryZoneSet?.beginX || null;
            }
            if (!entryZone1BeginX && zoneMap.entryZoneConfigEntities?.entry1) {
                const entryEntities = zoneMap.entryZoneConfigEntities.entry1;
                const entryZoneSet = entityResolver_1.EntityResolver.resolveZoneEntitySet(entityMappings, prefix, 'entryZoneConfigEntities', 'entry1', entryEntities);
                entryZone1BeginX = entryZoneSet?.beginX || null;
            }
            if (entryZone1BeginX) {
            ...
            logger_1.logger.error({ error }, 'Failed to check zone availability');
    ...
    router.get('/:deviceId/zones', async (req, res) => {
        ...
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to read zones');
    ...
    router.post('/:deviceId/zones', async (req, res) => {
        ...
        const entityNamePrefix = req.body?.entityNamePrefix ?? req.body?.entity_name_prefix;
        const zones = req.body?.zones ?? [];
        const entityMappings = req.body?.entityMappings;
        if (!profileId) {
        ...
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to apply zones');
    ...
    router.get('/:deviceId/polygon-mode', async (req, res) => {
        ...
        if (entityMappingsJson && typeof entityMappingsJson === 'string') {
            try {
                ...
                logger_1.logger.warn('Invalid entityMappings JSON in polygon-mode query');
            }
        }
        try {
            // Try device-level mapping first, then EntityResolver for the switch
            let switchEntityId = null;
            if (hasDeviceMapping) {
                switchEntityId = deviceEntityService_1.deviceEntityService.getEntityId(deviceId, 'polygonZonesEnabled');
            }
            if (!switchEntityId) {
                switchEntityId = entityResolver_1.EntityResolver.resolve(entityMappings, entityNamePrefix, 'polygonZonesEnabledEntity', entityTemplate);
            }
            if (!switchEntityId) {
                // No toggle entity: check for polygon zone entities (polygon-only firmware)
                let polygonZoneEntity = null;
                if (hasDeviceMapping) {
                    polygonZoneEntity = deviceEntityService_1.deviceEntityService.getPolygonZoneEntity(deviceId, 'polygon', 1);
                }
                if (!polygonZoneEntity && entityMap?.polygonZoneEntities?.zone1) {
                    polygonZoneEntity = entityResolver_1.EntityResolver.resolvePolygonZoneEntity(entityMappings, entityNamePrefix, 'polygonZoneEntities', 'zone1', entityMap.polygonZoneEntities.zone1);
                }
            ...
            logger_1.logger.info({ entityId: switchEntityId, entityNamePrefix, usedDeviceMapping: hasDeviceMapping }, 'Checking polygon mode entity');
            ...
            if (!state) {
                logger_1.logger.info({ entityId: switchEntityId }, 'Polygon mode entity not found - feature not available');
            ...
            logger_1.logger.info({ entityId: switchEntityId, supported: true, enabled, rawState: state.state }, 'Polygon mode status determined');
            return res.json({ supported: true, enabled, entityId: switchEntityId, controllable: true });
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Failed to get polygon mode status');
    ...
    router.post('/:deviceId/polygon-mode', async (req, res) => {
        ...
        const entityNamePrefix = req.body?.entityNamePrefix;
        ...
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to set polygon mode');
    ...
    router.get('/:deviceId/polygon-zones', async (req, res) => {
        ...
        try {
            ...
            logger_1.logger.error({ error }, 'Failed to read polygon zones');
    ...
    router.post('/:deviceId/polygon-zones', async (req, res) => {
        ...
        try {
            const warnings = [];
            const resolvePolygonEntity = (type, key, template) => {
                const indexMatch = key.match(/(\d+)/);
                const index = indexMatch ? parseInt(indexMatch[1], 10) : 1;
                const hasDeviceMapping = deviceMappingStorage_1.deviceMappingStorage.hasMapping(deviceId);
                if (hasDeviceMapping) {
                ...
                const groupKey = type === 'polygon' ? 'polygonZoneEntities'
            ...
            const polygonMap = entityMap?.polygonZoneEntities;
            const polygonExclusionMap = entityMap?.polygonExclusionEntities;
            const polygonEntryMap = entityMap?.polygonEntryEntities;
            ...
            const entityIds = resolvedEntities.map((entry) => entry.entityId).filter(Boolean);
            if (entityIds.length > 0) {
                ...
                try {
                    ...
                    logger_1.logger.warn({ error }, 'Failed to load entity registry for polygon zone checks');
                }
                ...
                try {
                    ...
                    logger_1.logger.warn({ error }, 'Failed to load entity states for polygon zone checks');
                }
            ...
            logger_1.logger.error({ error }, 'Failed to apply polygon zones');
