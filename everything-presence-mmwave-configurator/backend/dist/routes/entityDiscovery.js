...
exports.createEntityDiscoveryRouter = void 0;
const express_1 = require("express");
const entityDiscovery_1 = require("../domain/entityDiscovery");
const logger_1 = require("../logger");
const createEntityDiscoveryRouter = (deps) => {
    const router = (0, express_1.Router)();
    const { readTransport, profileLoader } = deps;
    const discoveryService = new entityDiscovery_1.EntityDiscoveryService(readTransport, profileLoader);
    /**
    ...
    router.get('/:deviceId/discover-entities', async (req, res) => {
        ...
        try {
            ...
            logger_1.logger.error({ error, deviceId, profileId }, 'Entity discovery failed');
    ...
    router.get('/:deviceId/entities', async (req, res) => {
        ...
        try {
            ...
            logger_1.logger.error({ error, deviceId }, 'Failed to get device entities');
            return res.status(500).json({
    ...
    router.post('/:deviceId/validate-mappings', async (req, res) => {
        ...
        try {
            const result = await discoveryService.validateMappings(mappings, deviceId);
            return res.json(result);
        }
        catch (error) {
            logger_1.logger.error({ error, deviceId }, 'Mapping validation failed');
            return res.status(500).json({
    ...
    router.post('/:deviceId/discover-and-save', async (req, res) => {
        ...
        try {
            ...
            logger_1.logger.error({ error, deviceId, profileId }, 'Discover and save failed');
            return res.status(500).json({
...
exports.createEntityDiscoveryRouter = createEntityDiscoveryRouter;
