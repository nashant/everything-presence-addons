...
class HeatmapService {
     ...
     */
    async generateHeatmap(entityNamePrefix, hours, resolution, zones, entityMappings, deviceId) {
     ...
     */
    getTrackingEntities(entityNamePrefix, entityMappings, deviceId) {
        const entities = [];
        const targetInfo = [];
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            if (hasDeviceMapping && deviceId) {
                const targetSet = deviceEntityService_1.deviceEntityService.getTargetEntities(deviceId, i);
                if (targetSet) {
            ...
            if (!xEntity) {
                xEntity = entityResolver_1.EntityResolver.resolveTargetEntity(entityMappings, entityNamePrefix, i, 'x');
            }
            if (!yEntity) {
                yEntity = entityResolver_1.EntityResolver.resolveTargetEntity(entityMappings, entityNamePrefix, i, 'y');
            }
            if (xEntity)
    ...
    async fetchHistory(entityIds, hours) {
        ...
        logger_1.logger.debug({ url: url.toString(), entityCount: entityIds.length }, 'Fetching HA history');
        ...
        if (!response.ok) {
            ...
            logger_1.logger.error({ status: response.status, error: text }, 'Failed to fetch HA history');
            throw new Error(`Failed to fetch history: ${response.status}`);
        }
        const data = await response.json();
        logger_1.logger.debug({ entityCount: data.length }, 'Fetched HA history');
        return data;
    }
    /**
     ...
     */
    correlateCoordinates(history, targetInfo) {
        const points = [];
        ...
        logger_1.logger.debug({ pointCount: points.length }, 'Correlated coordinate pairs');
        return points;
    }
    /**
     * Bucket points into grid cells and calculate intensities.
     */
    bucketPoints(points, resolution) {
        const grid = new Map();
     ...
     */
    calculateZoneStats(points, zones) {
        const stats = [];
        // Track points that are in at least one zone
        const pointsInAnyZone = new Set();
    ...
    pointInZone(point, zone) {
        if ((0, types_1.isZonePolygon)(zone)) {
     ...
     */
    calculateHourlyBreakdown(points) {
     ...
     */
    calculateAveragePosition(points) {
        if (points.length === 0) {
            return undefined;
        }
        let sumX = 0;
     ...
     */
    emptyResponse(resolution, hours) {
        const endTime = new Date();
        const startTime = new Date(endTime.getTime() - hours * 60 * 60 * 1000);
        return {
            resolution,
