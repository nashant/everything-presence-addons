...
class MigrationServiceImpl {
     ...
     */
    flattenZoneEntities(mappings, zones, prefix) {
        if (!zones)
            return;
     ...
     */
    flattenPolygonEntities(mappings, polygons, prefix) {
        if (!polygons)
            return;
        const keyToIndex = {
            zone1: 1, zone2: 2, zone3: 3, zone4: 4,
     ...
     */
    flattenTrackingTargets(mappings, targets) {
        if (!targets)
            return;
     ...
     */
    async migrateRoomToDeviceMapping(room) {
        ...
        if (existingMapping) {
            ...
            if (existingConfirmed) {
                logger_1.logger.info({ deviceId, roomId: room.id }, 'Device mapping was confirmed by user, skipping migration');
                return {
                    migrated: false,
            ...
            const existingBetter = existingScore > newScore ||
            ...
            if (existingBetter) {
                logger_1.logger.info({ deviceId, roomId: room.id, existingScore, newScore, existingMappingCount, newMappingCount }, 'Device mapping already exists with equal/better quality, skipping');
                return {
                    migrated: false,
            ...
            logger_1.logger.info({ deviceId, roomId: room.id, existingScore, newScore, existingMappingCount, newMappingCount }, 'Overwriting device mapping with better quality from room');
        }
        ...
        await deviceMappingStorage_1.deviceMappingStorage.saveMapping(deviceMapping);
        logger_1.logger.info({ deviceId, roomId: room.id, mappingCount: Object.keys(flatMappings).length }, 'Successfully migrated room entity mappings to device storage');
        return {
            migrated: true,
            deviceId,
            roomId: room.id,
    ...
    async migrateAllOnStartup() {
        const rooms = storage_1.storage.listRooms();
        const results = [];
        let migratedCount = 0;
        let skippedCount = 0;
        let errorCount = 0;
        logger_1.logger.info({ roomCount: rooms.length }, 'Starting entity mapping migration check');
        for (const room of rooms) {
            ...
            try {
                ...
                logger_1.logger.error({ error: error.message, roomId: room.id }, 'Failed to migrate room entity mappings');
            }
        }
        const summary = {
        ...
        logger_1.logger.info({ ...summary, results: undefined }, 'Entity mapping migration complete');
        return summary;
    }
    /**
    ...
    async dryRunMigration() {
        ...
        for (const room of rooms) {
            ...
            const existingMapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(room.deviceId);
            if (existingMapping) {
                const existingScore = existingMapping.autoMatchedCount ?? 0;
                const newScore = room.entityMappings?.autoMatchedCount ?? 0;
                const existingMappingCount = Object.keys(existingMapping.mappings).length;
...
exports.migrationService = new MigrationServiceImpl();
