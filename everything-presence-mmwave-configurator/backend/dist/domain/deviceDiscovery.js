...
class DeviceDiscoveryService {
    ...
    async getEntityNamePrefixForDevice(deviceId) {
        try {
            ...
            let deviceEntity = deviceEntities.find((e) => e.entity_id?.includes('_occupancy') || e.entity_id?.includes('_presence'));
            // Fallback to any binary_sensor or sensor
            if (!deviceEntity) {
                deviceEntity = deviceEntities.find((e) => e.entity_id?.startsWith('binary_sensor.') || e.entity_id?.startsWith('sensor.'));
            }
            ...
            if (deviceEntity?.entity_id) {
                ...
                const entityId = deviceEntity.entity_id;
                ...
                const prefix = withoutDomain.replace(new RegExp([
            ...
            logger_1.logger.warn({ error: error.message, deviceId }, 'Failed to get entity prefix for device');
        }
        return undefined;
    }
    async discover() {
        ...
        const [devices, areas] = await Promise.all([
            ...
            this.readTransport.listAreaRegistry().catch((err) => {
                logger_1.logger.warn({ err }, 'Failed to fetch area registry, continuing without area info');
                return [];
            }),
        ]);
        if (!Array.isArray(devices)) {
            logger_1.logger.warn('Unexpected devices payload from HA');
            return [];
        }
        ...
        const filteredDevices = devices
            .map((d) => ({
            id: d.id,
            name: d.name_by_user || d.name || 'Unnamed device',
            manufacturer: normalizeManufacturer(d.manufacturer) || undefined,
            model: d.model,
            firmwareVersion: d.sw_version,
            areaId: d.area_id,
        }))
            .filter((d) => {
        ...
        const enrichedDevices = await Promise.all(filteredDevices.map(async (device) => {
