...
exports.deviceEntityService = void 0;
...
class DeviceEntityServiceImpl {
    constructor() {
        ...
        this.legacyFallbackWarned = new Set();
    }
    /**
     ...
     */
    getMapping(deviceId) {
        return deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
    }
    /**
     * Get a specific entity ID by key.
     * Returns null if not found.
     */
    getEntityId(deviceId, entityKey) {
     ...
     */
    getService(deviceId, serviceKey) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return null;
        const profile = this.getProfile(mapping.profileId);
        if (!profile?.services?.[serviceKey])
            return null;
        const namePrefix = this.getDeviceNamePrefix(deviceId);
        if (!namePrefix) {
            logger_1.logger.warn({ deviceId, serviceKey }, 'Unable to resolve device name prefix for service');
            return null;
        }
     ...
     */
    resolveServiceFromList(deviceId, serviceKey, availableServices) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return null;
        const profile = this.getProfile(mapping.profileId);
        ...
        for (const full of availableServices) {
            ...
            if (domain !== def.domain)
                continue;
            if (!this.serviceMatchesTemplate(def.template, service))
                continue;
        ...
        matches.sort();
        if (matches.length > 1) {
            logger_1.logger.warn({ deviceId, serviceKey, matches }, 'Multiple services matched; using first sorted match');
        }
        return {
            key: serviceKey,
            domain: def.domain,
    ...
    getDeviceNamePrefix(deviceId) {
        ...
        const countsFromOriginal = new Map();
        const countsFromUnique = new Map();
        const countsFromEntityId = new Map();
        for (const [key, entityId] of Object.entries(mapping.mappings)) {
            ...
            if (!fallbackCandidate)
                continue;
            countsFromEntityId.set(fallbackCandidate, (countsFromEntityId.get(fallbackCandidate) ?? 0) + 1);
        }
        ...
        if (sorted.length > 1) {
            logger_1.logger.warn({ deviceId, source, candidates: sorted.map(([name, count]) => ({ name, count })) }, 'Multiple name prefixes detected; using most common');
        }
        return sorted[0][0];
    }
    /**
     ...
     */
    getEntitiesByCategory(deviceId, category) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return [];
        const profile = this.getProfile(mapping.profileId);
        ...
        if (profile.entities) {
            const results = [];
            for (const [key, def] of Object.entries(profile.entities)) {
    ...
    extractNamePrefixFromObjectId(template, objectId) {
    ...
    serviceMatchesTemplate(template, service) {
     ...
     */
    extractNamePrefix(template, entityId) {
        const match = template.match(/^([a-z_]+)\.\$\{name\}(.*)$/);
        if (!match)
            return null;
     ...
     */
    getZoneEntitySet(deviceId, zoneType, zoneIndex) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return null;
        const profile = this.getProfile(mapping.profileId);
        ...
        if (profile.entities) {
            ...
            for (const coord of ['beginX', 'endX', 'beginY', 'endY']) {
        ...
        if (!result.beginX || !result.endX || !result.beginY || !result.endY) {
            logger_1.logger.debug({ deviceId, zoneType, zoneIndex, result }, 'Zone missing required coordinates');
            return null;
        }
        return result;
    }
     ...
     */
    getZoneKeyPatterns(zoneType, zoneIndex) {
        if (zoneType === 'regular') {
            return {
     ...
     * Uses entities metadata when available, falls back to key patterns.
     */
    getPolygonZoneEntity(deviceId, zoneType, zoneIndex) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return null;
        const profile = this.getProfile(mapping.profileId);
     ...
     */
    getTargetEntities(deviceId, targetIndex) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        if (!mapping)
            return null;
        const profile = this.getProfile(mapping.profileId);
        ...
        if (profile?.entities) {
            ...
            const properties = ['x', 'y', 'speed', 'distance', 'angle', 'resolution', 'active'];
        ...
        if (!result.x || !result.y) {
            logger_1.logger.debug({ deviceId, targetIndex, result }, 'Target missing required x/y coordinates');
            return null;
        }
        return result;
    }
    ...
    getSettingsGrouped(deviceId) {
        ...
        if (profile.entities) {
            ...
            const settingsEntities = entityMap?.settingsEntities;
            if (settingsEntities) {
     ...
     */
    getEntityUnit(deviceId, entityKey) {
        const mapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
    ...
    getProfile(profileId) {
        if (!this.profileLoader) {
            logger_1.logger.warn('DeviceEntityService: ProfileLoader not set');
            return null;
        }
        return this.profileLoader.getProfileById(profileId) ?? null;
    }
    /**
     * Resolve an entity with fallback to legacy room mappings.
     * Logs a rate-limited warning when using legacy fallback.
     */
    resolveWithFallback(deviceId, entityKey, legacyMappings) {
        // Try device mapping first (preferred)
        const deviceMapping = deviceMappingStorage_1.deviceMappingStorage.getMapping(deviceId);
        ...
        if (legacyMappings) {
            if (!this.legacyFallbackWarned.has(deviceId)) {
                logger_1.logger.warn({ deviceId, entityKey }, 'Using legacy room mappings - device needs migration');
     ...
     */
    clearFallbackWarnings() {
        this.legacyFallbackWarned.clear();
    }
}
// Export singleton instance
exports.deviceEntityService = new DeviceEntityServiceImpl();
